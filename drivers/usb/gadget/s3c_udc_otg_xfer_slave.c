				   ...
				   struct s3c_request *req, int max)
{
	...
	if(ep_index(ep) == EP0_CON) {
		writel((1<<19)|(length<<0), (u32) S3C_UDC_OTG_DIEPTSIZ(EP0_CON));

		in_ctrl =  readl(S3C_UDC_OTG_DIEPCTL(EP0_CON));
		writel(DEPCTL_EPENA|DEPCTL_CNAK|(EP2_IN<<11)| in_ctrl, (u32) S3C_UDC_OTG_DIEPCTL(EP0_CON));

		DEBUG_EP0("%s:(DIEPTSIZ0):0x%x, (DIEPCTL0):0x%x, (GNPTXSTS):0x%x\n", __FUNCTION__,
			readl(S3C_UDC_OTG_DIEPTSIZ(EP0_CON)),readl(S3C_UDC_OTG_DIEPCTL(EP0_CON)),
			readl(S3C_UDC_OTG_GNPTXSTS));

		udelay(30);

	} else if ((ep_index(ep) == EP2_IN)) {
		writel((1<<19)|(length<<0), S3C_UDC_OTG_DIEPTSIZ(EP2_IN));

		in_ctrl =  readl(S3C_UDC_OTG_DIEPCTL(EP2_IN));
		writel(DEPCTL_EPENA|DEPCTL_CNAK|(EP2_IN<<11)| in_ctrl, (u32) S3C_UDC_OTG_DIEPCTL(EP2_IN));

		DEBUG_IN_EP("%s:(DIEPTSIZ2):0x%x, (DIEPCTL2):0x%x, (GNPTXSTS):0x%x\n", __FUNCTION__,
			readl(S3C_UDC_OTG_DIEPTSIZ(EP2_IN)),readl(S3C_UDC_OTG_DIEPCTL(EP2_IN)),
			readl(S3C_UDC_OTG_GNPTXSTS));
		...
		writel((1<<19)|(length<<0), S3C_UDC_OTG_DIEPTSIZ(EP3_IN));

		in_ctrl =  readl(S3C_UDC_OTG_DOEPCTL(EP3_IN));
		writel(DEPCTL_EPENA|DEPCTL_CNAK|(EP2_IN<<11)| in_ctrl, (u32) S3C_UDC_OTG_DOEPCTL(EP3_IN));

		DEBUG_IN_EP("%s:(DIEPTSIZ3):0x%x, (DIEPCTL3):0x%x, (GNPTXSTS):0x%x\n", __FUNCTION__,
			readl(S3C_UDC_OTG_DIEPTSIZ(EP3_IN)),readl(S3C_UDC_OTG_DOEPCTL(EP3_IN)),
			readl(S3C_UDC_OTG_GNPTXSTS));
	...
	if (intr_status & INT_RX_FIFO_NOT_EMPTY) {
		...
		if (fifoCntByte) {
			...
			if (packet_status == SETUP_PKT_RECEIVED)  {
				...
				if(ep_num == EP1_OUT) {
					ep_ctrl = readl(S3C_UDC_OTG_DOEPCTL(EP1_OUT));
					DEBUG_ISR("      => A Bulk OUT data packet received : %d bytes, (DOEPCTL1):0x%x\n",
						fifoCntByte, ep_ctrl);
					s3c_out_epn(dev, 1);
					gintmsk = readl(S3C_UDC_OTG_GINTMSK);
					writel(ep_ctrl | DEPCTL_CNAK, S3C_UDC_OTG_DOEPCTL(EP1_OUT));
			...
			if (packet_status == SETUP_TRANSACTION_COMPLETED) {
				...
				writel(ep_ctrl | DEPCTL_CNAK, S3C_UDC_OTG_DOEPCTL(EP0_CON));
			...
			if((csr & INT_RX_FIFO_NOT_EMPTY) &&
			   (read_fifo(ep, req) == 1))
				...
				DEBUG("++++ OUT-list_add_taill::req=%p, DOEPCTL1:0x%x\n",
					req, readl(S3C_UDC_OTG_DOEPCTL(EP1_OUT)));
		}
	}
...
static void udc_set_address(struct s3c_udc *dev, unsigned char address)
{
	...
	writel(DEPCTL_EPENA|DEPCTL_CNAK|ctrl, S3C_UDC_OTG_DIEPCTL(EP0_CON)); /* EP0: Control IN */
...
config_change:
			...
			writel(DEPCTL_EPENA|DEPCTL_CNAK|ep_ctrl, S3C_UDC_OTG_DIEPCTL(EP0_CON)); /* EP0: Control IN */

			// For Startng EP1 on this new configuration
			ep_ctrl = readl(S3C_UDC_OTG_DOEPCTL(EP1_OUT));
			writel(DEPCTL_EPDIS|DEPCTL_CNAK|DEPCTL_BULK_TYPE|DEPCTL_USBACTEP|ep_ctrl, S3C_UDC_OTG_DOEPCTL(EP1_OUT)); /* EP1: Bulk OUT */

			// For starting EP2 on this new configuration
			ep_ctrl = readl(S3C_UDC_OTG_DIEPCTL(EP2_IN));
			writel(DEPCTL_BULK_TYPE|DEPCTL_USBACTEP|ep_ctrl, S3C_UDC_OTG_DIEPCTL(EP2_IN)); /* EP2: Bulk IN */

			// For starting EP3 on this new configuration
			ep_ctrl = readl(S3C_UDC_OTG_DOEPCTL(EP3_IN));
			writel(DEPCTL_BULK_TYPE|DEPCTL_USBACTEP|ep_ctrl, S3C_UDC_OTG_DOEPCTL(EP3_IN)); /* EP3: INTR IN */

			DEBUG_SETUP("%s:(DOEPCTL1):0x%x, (DIEPCTL2):0x%x, (DIEPCTL3):0x%x\n",
				__FUNCTION__,
				readl(S3C_UDC_OTG_DOEPCTL(EP1_OUT)),
				readl(S3C_UDC_OTG_DIEPCTL(EP2_IN)),
				readl(S3C_UDC_OTG_DOEPCTL(EP3_IN)));
