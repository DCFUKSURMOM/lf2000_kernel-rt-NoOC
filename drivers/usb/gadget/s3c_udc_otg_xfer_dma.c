...
static int setdma_rx(struct s3c_ep *ep, struct s3c_request *req)
{
	...
	struct device *dev = &ep->dev->dev->dev;	/* jhkim : add */

	buf = req->req.buf + req->req.actual;
	prefetchw(buf);

	length = req->req.length - req->req.actual;
	/* jhkim : change from dma_cache_maint(buf, length, DMA_FROM_DEVICE); */
	dma_unmap_single(dev, (u32)virt_to_phys(buf), length, DMA_FROM_DEVICE);
...
static int setdma_tx(struct s3c_ep *ep, struct s3c_request *req)
{
	...
	/* jhkim : change from dma_cache_maint(buf, length, DMA_TO_DEVICE); */
	dma_unmap_single(dev, (u32)virt_to_phys(buf), length, DMA_TO_DEVICE);
...
static void complete_rx(struct s3c_udc *dev, u8 ep_num)
{
	...
	struct device *device = &dev->dev->dev;	/* jhkim : add */
	...
	/* jhkim : change from dma_cache_maint(req->req.buf, req->req.length, DMA_FROM_DEVICE); */
	dma_unmap_single(device, (u32)virt_to_phys(req->req.buf), req->req.length, DMA_FROM_DEVICE);
...
static irqreturn_t s3c_udc_irq(int irq, void *_dev)
{
	...
	if (intr_status & INT_EARLY_SUSPEND) {
		...
		DEBUG_ISR("\tEarly suspend interrupt: (DSTS): 0x%x\n", usb_status);
		writel(INT_EARLY_SUSPEND, S3C_UDC_OTG_GINTSTS);

		//Check for erratic error bit
		if( usb_status & DSTS_ERRTICERR )
		{		
			// modify charles
...
static __inline__ int s3c_fifo_read(struct s3c_ep *ep, u32 *cp, int max)
{
	...
	/* jhkim : change from dma_cache_maint(&usb_ctrl, bytes, DMA_FROM_DEVICE); */
	dma_unmap_single(dev, virt_to_phys(&usb_ctrl), bytes, DMA_FROM_DEVICE);
		...
		struct usb_ctrlrequest *crq)
{
	...
	/* jhkim : change from dma_cache_maint(&g_status, 2, DMA_TO_DEVICE); */
	dma_unmap_single(device, (u32)virt_to_phys(&g_status), 2, DMA_TO_DEVICE);
