...
#include <linux/clk.h>
#include <asm/mach/map.h>		/* jhkim : change frorm mach/map.h 	*/
#include <mach/reg_otg.h>		/* jhkim : change frorm plag/regs-otg.h */
...
{
	/* jhkim : add "#if defined(CONFIG_PM)" */
...
/* DMA mode, jhkim : change from CONFIG_USB_GADGET_S3C_OTGD_DMA_MODE */
#if	defined(CONFIG_USB_GADGET_NEXELL_OTGD_DMA_MODE)
#define OTG_DMA_MODE		1

#elif	defined(CONFIG_USB_GADGET_NEXELL_OTGD_SLAVE_MODE) /* Slave mode */
		...
		int (*bind)(struct usb_gadget *))
{
	...
	if (!driver
	    ...
	    || !bind /* jhkim : change from !driver->bind */
...
/*
 * jhkim : change from EXPORT_SYMBOL(usb_gadget_register_driver);
 */
...
static void set_max_pktsize(struct s3c_udc *dev, enum usb_device_speed speed)
{
	...
	dev->ep[3].ep.maxpacket = ep_fifo_size2;
	dev->ep[4].ep.maxpacket = ep_fifo_size2;
	/* jhkim : remove
...
static struct s3c_udc memory = {
	...
	.gadget = {
		   ...
		   .name = driver_name,
		   .dev = {
			   .init_name 	= "gadget", /* jhkim: change from .bus_id = "gadget" */
			   .release 	= nop_release,
			   },
		   },
...
static int s3c_udc_resume(struct platform_device *pdev)
{
        ...
        if (dev->driver) {
        		/* jhkim : change * clk_enable(otg_clock); */
                otg_clk_enable();
                udc_reinit(dev);
                enable_irq(IRQ_OTG);
                udc_enable(dev);
...
#else
		.name	= "otg-gadget",	/* jhkim change : .name	= "s3c-usbgadget" */
#endif
