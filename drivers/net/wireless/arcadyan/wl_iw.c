...
int wl_iw_if_ip_changed(unsigned long event)
{

	if (event == NETDEV_UP) {
		WL_TRACE_COEX(("%s BTCOEX. eth0 IP is up, pid:%x\n",
		...
		if (g_bt->bt_state != BT_DHCP_IDLE) {
			...
			WL_TRACE_COEX(("%s wake up BTCOEX stm, bt->bt_state:%d\n",
				__FUNCTION__, g_bt->bt_state));
			up(&g_bt->tsk_ctl.sema);
		} else {
			WL_TRACE_COEX(("BT is not active, do noting\n"));
		}
	}
	return 0;
}
...
static int set_btc_esco_params(struct net_device *dev, bool trump_sco)
{
	...
	if (trump_sco) { 
		...
		WL_TRACE_COEX(("Do new SCO/eSCO coex algo {restore saved} \n"));
...
)
{
	...
	WL_TRACE_COEX(("%s: enter, pid:%x\n",
		__FUNCTION__, current->pid));

	
	strncpy((char *)&btcoexmode_val, extra + strlen("BTCOEXMODE") +1, 1);

	if (strnicmp((char *)&btcoexmode_val, "1", strlen("1")) == 0) {
	...
	else if (strnicmp((char *)&btcoexmode_val, "2", strlen("2")) == 0) {
...
static int set_ap_cfg(struct net_device *dev, struct ap_profile *ap)
{
	...
	if (strlen(ap->country_code)) {
		...
		if (dev_iw_iovar_setbuf(dev, "country", &cspec, sizeof(cspec), \
			smbuf, sizeof(smbuf)) >= 0) {
				WL_ERROR(("%s: set country for %s as %s rev %d is OK\n", \
					__FUNCTION__, ap->country_code, cspec.ccode, cspec.rev));
				dhd_bus_country_set(dev, &cspec);
		}
	} else {
...
wl_iw_event(struct net_device *dev, wl_event_msg_t *e, void* data)
{
	...
	WL_ASSOC((" WLC_E DISASSOC or DEAUTH ind %d\n", event_type));
#if defined(SOFTAP)
