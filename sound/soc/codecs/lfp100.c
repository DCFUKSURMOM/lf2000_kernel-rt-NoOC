...
static int lfp100_set_dai_fmt(struct snd_soc_dai *codec_dai,
		unsigned int format)
{
	printk(KERN_INFO "%s: %s fmt=%08x\n", __FUNCTION__, codec_dai->name, format);
	switch (format & SND_SOC_DAIFMT_FORMAT_MASK) {
	case SND_SOC_DAIFMT_I2S:
		lfp100_write_reg(LFP100_FORMAT_PW, 0xE6);
		break;
	case SND_SOC_DAIFMT_LEFT_J:
		lfp100_write_reg(LFP100_FORMAT_PW, 0xC6);
		break;
	case SND_SOC_DAIFMT_RIGHT_J:
		lfp100_write_reg(LFP100_FORMAT_PW, 0x96);
		break;
	default:
		return -EINVAL;
	}
...
static int lfp100_dai_mute(struct snd_soc_dai *dai, int mute)
{
	...
	printk(KERN_INFO "%s: %s, mute=%d, playback=%d, capture=%d\n", __FUNCTION__, dai->name,
...
//			lfp100_write_reg(LFP100_MICGAIN, 0x00);		// unmute mic gain
//			lfp100_write_reg(LFP100_A_CONTROL, 0x01);	// enable mic in
//			reg = lfp100_read_reg(LFP100_A_CONTROL);
//			lfp100_write_reg(LFP100_A_CONTROL, reg | LFP100_A_CONTROL_MIC_EN);
			break;
		case SNDRV_PCM_TRIGGER_STOP:
//			lfp100_write_reg(LFP100_MICGAIN, 0x40);		// mute mic gain
//			lfp100_write_reg(LFP100_A_CONTROL, 0x00);	// disable mic in
//			reg = lfp100_read_reg(LFP100_A_CONTROL);
//			lfp100_write_reg(LFP100_A_CONTROL, reg & ~LFP100_A_CONTROL_MIC_EN);
			break;
		}
...
//	SOC_SINGLE("Microphone Gain", LFP100_MICGAIN, 0, 0x7F, 0),
//	SOC_SINGLE_TLV("Microphone Gain", LFP100_MICGAIN, 0, 0x7F, 0, micgain_tlv),
